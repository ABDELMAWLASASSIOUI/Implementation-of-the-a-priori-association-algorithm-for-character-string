...
public class AprioriAlgorithm {
    ...
    public void loadTransactionsFromFile(String filePath) throws IOException {
        List<String> lines = FileHandler.loadTransactions(filePath);
    ...
    public void generateAssociationRules(double minConfidence) {  //Cette méthode génère des règles d'association basées sur les itemsets fréquents
        rules.clear();
        for (Map.Entry<Set<String>, Integer> entry : itemsets.entrySet()) {
            Set<String> itemset = entry.getKey();//Extraction de l'itemset
            int supportCount = entry.getValue();//et de son support
            generateRulesForItemset(itemset, supportCount, minConfidence);
        }

        System.out.println("\n Règles d'association :");
        for (Map.Entry<String, Double> rule : rules.entrySet()) {
            System.out.println(rule.getKey() + " (Confiance: " + String.format("%.2f", rule.getValue()) + ")");
        }
    }
    ...
    // Méthode pour éliminer les espaces dans chaque item
